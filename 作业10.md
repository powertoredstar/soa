...
![项目创建](image/第10次作业-项目创建.png)

### 截图2：Maven 依赖加载

![Maven依赖](image/第10次作业-Maven依赖.png)

### 截图3：访问首页

![首页](image/第10次作业-首页.png)

### 截图4：API 返回 JSON

![API JSON](image/第10次作业-API返回JSON.png)

### 截图5：动态页面测试

![动态页面](image/第10次作业-动态页面.png)
...
1. **在 Eclipse 中导入项目**
   ...
   - 选择 `eclipse-workspace/homework10-spring-mvc` 目录

2. **下载 Maven 依赖**
   - 右键项目 → `Run As` → `Maven install`
   - 等待依赖下载完成（spring-webmvc、jackson-databind 等）
...
5. **截图**
   - 项目创建成功截图 → 保存为 `image/第10次作业-项目创建.png`
   - Maven 依赖加载截图 → 保存为 `image/第10次作业-Maven依赖.png`
   - 首页访问截图 → 保存为 `image/第10次作业-首页.png`
   - API 返回 JSON 截图 → 保存为 `image/第10次作业-API返回JSON.png`
   - 动态页面测试截图 → 保存为 `image/第10次作业-动态页面.png`

6. **提交到 Git**
   ```bash
   git add eclipse-workspace/homework10-spring-mvc/ 作业10.md image/
   git commit -m "完成作业10：Maven + Spring MVC RESTful"
   git push
   ```
