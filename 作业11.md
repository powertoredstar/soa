...
![项目创建](image/第11次作业-项目创建.png)

### 截图2：管理界面首页

![管理界面](image/第11次作业-管理界面.png)

### 截图3：新增学生

![新增学生](image/第11次作业-新增学生.png)

### 截图4：编辑学生

![编辑学生](image/第11次作业-编辑学生.png)

### 截图5：删除学生

![删除学生](image/第11次作业-删除学生.png)

### 截图6：搜索学生

![搜索学生](image/第11次作业-搜索学生.png)

### 截图7：API 返回 JSON

![API JSON](image/第11次作业-API返回JSON.png)
...
1. **在 Eclipse 中导入项目**
   ...
   - 选择 `eclipse-workspace/homework11-spring-boot` 目录

2. **下载 Maven 依赖**
   - 右键项目 → `Run As` → `Maven install`
   - 等待依赖下载完成（spring-boot-starter-web 等）

3. **配置 Tomcat Server Runtime**（如未配置）
   - 右键项目 → `Properties` → `Java Build Path` → `Libraries` → `Add Library` → `Server Runtime` → `Tomcat 9`

4. **运行项目**
   - 右键项目 → `Run As` → `Run on Server` → 选择 Tomcat 9

5. **截图**
   - 项目创建成功截图 → 保存为 `image/第11次作业-项目创建.png`
   - 管理界面首页截图 → 保存为 `image/第11次作业-管理界面.png`
   - 新增学生截图 → 保存为 `image/第11次作业-新增学生.png`
   - 编辑学生截图 → 保存为 `image/第11次作业-编辑学生.png`
   - 删除学生截图 → 保存为 `image/第11次作业-删除学生.png`
   - 搜索学生截图 → 保存为 `image/第11次作业-搜索学生.png`
   - API 返回 JSON 截图 → 保存为 `image/第11次作业-API返回JSON.png`

6. **提交到 Git**
   ```bash
   git add eclipse-workspace/homework11-spring-boot/ 作业11.md image/
   git commit -m "完成作业11：Spring Boot + RESTful 学生管理系统"
   git push
   ```
